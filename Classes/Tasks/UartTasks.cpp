...
namespace Tasks {
    UartTasks::UartTasks() {
        txQueue = xQueueCreateStatic(TX_QUEUE_DEPTH, sizeof(LLA::UartString), txQueueStorageArea,
                                             &txQueueControlBlock);
        txSemaphore = xSemaphoreCreateBinaryStatic(&txSemaphoreBuffer);
        rxQueue = xQueueCreateStatic(RX_QUEUE_DEPTH, sizeof(LLA::UartString), rxQueueStorageArea,
                                             &rxQueueControlBlock);
        //registering queues for statistics purposes (not necessary for them to work properly)
        vQueueAddToRegistry(txQueue, StringLiterals::UartTxQueueName);
        vQueueAddToRegistry(rxQueue, StringLiterals::UartRxQueueName);
    }
    ...
    void UartTasks::addToTxQueue(std::string newMessage) {
        ...
        xQueueSendToBack(txQueue, (void *) &string, 0);
    }

    void UartTasks::addToTxQueueFromISR(std::string newMessage) {
        LLA::UartString string;
        string.convertFromString(newMessage);
        xQueueSendToBackFromISR(txQueue, (void *) &string, 0);
    }

    void UartTasks::addToRxQueueFromISR(LLA::UartString newMessage) {
        xQueueSendToBackFromISR(rxQueue, (void *) &newMessage, 0);
    }

    void UartTasks::txTask() {
        while (1) {
            xQueueReceive(txQueue, &txBuffer, portMAX_DELAY);
    ...
    void UartTasks::rxTask() {
        while (1) {
            xQueueReceive(rxQueue, &rxBuffer, portMAX_DELAY);
            std::string message;
